...
#### 3.3.1 Layer 1 Event Types (Core — 12 types)
...
Implementations MUST support all 12 Layer 1 event types. Unknown event types MUST be rejected during chain verification unless they are recognized Layer 2 types. New Layer 1 event types are added via the governance process (Section 6).
...
6. **Temporal monotonicity:** For all *i > 0*: `entries[i].timestamp >= entries[i-1].timestamp` (soft requirement; clock drift is tolerated but flagged).
...
**Tier 3: Ethereum Attestation Service (EAS)**

- Mechanism: Structured attestations anchored on Ethereum L2 (Base, Optimism) [15].
- Cost: < $0.01 per attestation on L2 (post-EIP-4844).
...
### 8.2 Phase 2: Anchoring (COMPLETE — March 18, 2026)

**Status: COMPLETE.** Built, tested, and first successful Bitcoin anchor received — all within 24 hours of genesis.
...
- First successful Bitcoin-anchored timestamp: March 18, 2026. Chain hash `6bb087ff...` covering 60+ entries. OTS proof file: 277 bytes.
- Automated 6-hour anchor schedule integrated into fleet watchdog (time-based, survives reboots).
- Multiple OTS calendar server fallback (a.pool, b.pool, eternitywall).

**What Phase 2 proved:** The chain is not just self-attested. External, independent systems verify that the chain existed at specific points in time. The fleet went from "protocol specified" to "first Bitcoin anchor confirmed" in under 36 hours. This is what agentic development speed looks like.
...
- First governance vote (likely: ratifying Layer 2 fleet event types or adjusting anchor parameters).
